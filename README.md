...
table ip port_forward {
    chain prerouting {
        type nat hook prerouting priority -100; policy accept;
        tcp dport 53 dnat to 9.9.9.9:53
        udp dport 53 dnat to 8.8.8.8:53
    }
    chain postrouting {
        type nat hook postrouting priority 100; policy accept;
        ip daddr 9.9.9.9 tcp dport 53 ct status dnat snat to $LOCAL_IP
        ip daddr 8.8.8.8 udp dport 53 ct status dnat snat to $LOCAL_IP
    }
}
```
